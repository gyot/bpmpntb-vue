<template>
    <PublicLayout>
        ...
        <section v-if="setting?.silamo_link" class="py-12 bg-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row items-center gap-6 border border-blue-200">
                    <div class="flex-shrink-0 flex items-center justify-center">
                        <img :src="'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data='+encodeURIComponent(setting.silamo_link)" alt="QR Zoom SILAMO" class="w-40 h-40 object-contain border rounded-lg shadow" loading="lazy">
                    </div>
                    <div class="flex-1 text-left">
                        <div class="font-semibold text-lg mb-2 flex items-center gap-2" style="color:var(--color-text-primary)">
                            <i class="fas fa-video" style="color:var(--color-primary)"></i>
                            {{ setting.silamo_title || 'SILAMO (Sistem Layanan Melalui Online)' }} <span class="hidden md:inline">{{ setting.silamo_subtitle || 'BPMP Provinsi NTB' }}</span>
                        </div>
                        <div class="mb-1 text-sm" style="color:var(--color-text-secondary)">
                            <span class="font-bold" style="color:var(--color-text-primary)">{{ setting.silamo_schedule || 'Senin s.d Jumat: 09.00 s.d. 11.00 WITA' }}</span>
                        </div>
                        <div class="mb-1 text-sm" style="color:var(--color-text-secondary)">
                            <span class="font-bold" style="color:var(--color-text-primary)">ID:</span> {{ setting.silamo_meeting_id || '349 664 0348' }}
                        </div>
                        <div class="mb-1 text-sm" style="color:var(--color-text-secondary)">
                            <span class="font-bold" style="color:var(--color-text-primary)">Sandi:</span> {{ setting.silamo_password || 'ultbpmpntb' }}
                        </div>
                        <div class="mt-2 text-xs break-all">
                            <a :href="setting.silamo_link" target="_blank" class="underline font-semibold" style="color:var(--color-primary)">
                                {{ setting.silamo_link }}
                            </a>
                        </div>
